...
function LoginPage() {
  ...
  return (
      ...
      <div className="bg-white/10 p-8 rounded-lg shadow-md backdrop-blur mx-3 space-y-8 max-w-md w-full ">
        <h2 className="text-2xl mb-4 font-semibold text-gray-800 text-center">Login</h2>
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            <label htmlFor="email" className="block text-sm font-medium text-white">Email</label>
            <input id="email" name="email" type="email" autoComplete="email" placeholder='rishi@example.com' required value={email} onChange={(e) => setEmail(e.target.value)} className="mt-1 p-2 block w-full border border-gray-300 rounded-md focus:outline-none focus:border-blue-500" />
          </div>
        ...
        <div className="mt-4 text-sm">
          <p>Doesn't have an account? <a href="#" className="font-bold text-black/50 hover:text-black/100">Register here</a></p>
        </div>
      </div>
      </div>
  );
